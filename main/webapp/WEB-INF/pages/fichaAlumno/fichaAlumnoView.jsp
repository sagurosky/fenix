			...
			$.ajax( {
			     ...
			     success: function(data) {
			           ...
			           $(data.sucursales).each(function() {
			            ...
			            $("select#sucursalRa2").html(options);
...
function hacerSubmit(){
	...
	//DMS aca esta la url del controlador
...
<body>
	<form:form name="alumnoForm" id="alumnoForm" commandName="perfilAlumno"
		enctype='multipart/form-data' modelAttribute="perfilAlumno"
		method="post">
		<input type="hidden" name="alumnoEstado" id="alumnoEstado"
			value="${perfilAlumno.estadoAlumno.id}" />
		<center>
			<br> <input type="button" onclick="hacerSubmit()"
				value="Guardar cambios"
				class="ui-state-default ui-corner-all modificar"></input>
			<c:if test="${urlRegreso != null}">
				<input type="button" value="Volver"
					class="ui-state-default ui-corner-all"
					onclick="location.href='..${urlRegreso}'; return false;" />
			</c:if>
		</center>
		<input type="hidden" name="actualizacionPBE" id="actualizacionPBE" />
		<input type="hidden" id="actualizacionPeriodoAsignacion"
			name="actualizacionPeriodoAsignacion" />
