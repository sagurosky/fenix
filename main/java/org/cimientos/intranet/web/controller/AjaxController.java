...
//			if (idSituacionEscolar == 4){
  ...
  /* Retorna responsable adulto por nombre.
    * Este metodo se utiliza para autocompletar las provincias
    *
    * @param response the response
    * @param texto the texto
    ...
    * @author cfigueroa
    */
    @RequestMapping("/ajax/buscarResponsableAdulto.do")
    public void buscarResponsable(HttpServletResponse response,
    		@RequestParam("name_startsWith") String texto) throws JSONException {
    	
    	JSONObject jSon = new JSONObject();
    	JSONArray responsables = new JSONArray();
    	List<ResponsableAdulto> listado = responsableAdultoSrv.obtenerResponsablesPorNombre(texto);
    	if(listado != null){
    		System.out.println("listado: "+listado.size());
    		for (ResponsableAdulto resp : listado){
    			JSONObject respObj = new JSONObject();
    			respObj.put("nombre", resp.getNombre());
    			respObj.put("apellido", resp.getApellido());
    			respObj.put("id", resp.getId());
    			
    			responsables.put(respObj);
    		}
    	}else{
    		JSONObject respObj = new JSONObject();
    		respObj.put("nombre", "no hay datos");
    		respObj.put("id", "");
