...
implements ResponsableDao{
	...
	@Override
	public List<ResponsableAdulto> obtenerResponsablesPorNombre(String texto) {
		Criteria criteria = getSession().createCriteria(getObjetoManejado());
		criteria.add(Restrictions.or(Restrictions.like("nombre", "%"+texto+"%"),Restrictions.like("apellido", "%"+texto+"%") ));
		criteria.addOrder(Order.asc("apellido"));
		List<ResponsableAdulto> lista = criteria.list();
		if(lista.size()>0){
			return lista;
		}else{
			return null;
