...
public class AlumnoDaoImpl extends SpringHibernateDao<PerfilAlumno> implements AlumnoDao{
	...
	public List<ExportarAlumnosDTO> obtenerAlumnosExportar(int firstResult, int maxResults,
		...
		if(padrino == null && nombreAlumno == null){
			...
			String sql ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
			...
			String sql2 = "select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, zona_cimientos zona,"
						 + " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
						 + " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
			...
			if(padrino != null){				
				sql3 ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
					+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
					+ " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
			...
			if(nombreAlumno != null && padrino != null){
				if(zona != null){
					sql3 ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con, persona per, escuela esc, localidad loc, zona_cimientos zona, beca beca,"
					...
					sql3 ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con, persona per, beca beca, "
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro"
			...
			if(nombreAlumno != null && padrino == null ){
				if(zona != null){
					sql3 ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo,  entrevista_seleccion sel, convocatoria con, persona per, zona_cimientos zona, escuela esc, localidad loc,"
					...
					sql3 ="select alum.*, sel.*, con.* from perfil_alumno alum, ciclo_programa ciclo, entrevista_seleccion sel, convocatoria con, persona per,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro where alum.id = sel.id_perfil_alumno" 
	...
	//DMS 10/10 implementacion con agregado de AnioEgreso
	public List<ExportarAlumnosDTO> obtenerAlumnosExportar(int firstResult, int maxResults,
            SortOrderEnum sortDirection, String sortCriterion,EstadoAlumno estadoAlumno, 
            ZonaCimientos zona, Convocatoria convocatoria, PerfilPadrino padrino, PerfilEA ea, String nombreAlumno,
            List<AnioEscolar> anios, Boolean anioAdicional, boolean becadosActivos, String eae,
            Escuela escuela,Integer[] aniosEgreso,Integer dni,String localidad, String provincia, Long idRa, Long idRr) {
		//System.out.println("eae: "+eae);
		String consulta = "";
		String sql3 = "";
		//DMS 19/10 si dni es null no llamo a la tabla persona porque ralentiza la busqueda
		String pers="";
		String prov="";
		String respAdul="";
		String respReg="";
		String beca="";
		if(dni!=null)pers=" persona pers, ";
		if(provincia!=null&&!provincia.equals(""))prov=" provincia prov, ";
	//	if(rr!=null&&!rr.equals(""))respReg=" perfilea pea, perfilrr prr, ";
		...
		if(padrino == null && nombreAlumno == null ){
			...
			String sql ="select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+prov+respAdul+respReg+" entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
						+ " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						+ " and alum.beca_id = beca.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id";
			
			String sql2 = "select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+prov+respAdul+respReg+"  entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, zona_cimientos zona,"
						 + " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
						 + " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						 + " and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id";
			...
			if(anios.size()>0){	
				...
				sql+=" and (";
				sql2+=" and (";
				for(AnioEscolar anio:anios)
				{
					sql += "  alum.anio_escolar= " + (anio.getId() -1)+" or ";
					sql2 += " alum.anio_escolar= " + (anio.getId() -1)+" or ";
				}
			...
			if(aniosEgreso!=null)
			{
				...
				sql+=" AND beca.ciclo=ciclo.id AND (";
				for (Integer anioEgreso : aniosEgreso) {
					
					
					sql+="  ("
						  	+	"("
							+	"(zona.eae='6/6'AND alum.anio_adicional=true AND  alum.anio_escolar=(1+CAST(ciclo.nombre AS SIGNED)+13-"+anioEgreso+"))"
							+	" or"
							+	" (zona.eae='6/6'AND alum.anio_adicional=false AND  alum.anio_escolar=(CAST(ciclo.nombre AS SIGNED)+13-"+anioEgreso+"))"
							+	")"
						  	
							+   "OR" 
							+  	"("
							+	"(zona.eae='7/5'AND alum.anio_adicional=true AND  alum.anio_escolar=(1+CAST(ciclo.nombre AS SIGNED)+12-"+anioEgreso+"))"
							+	"or"
							+	"(zona.eae='7/5'AND alum.anio_adicional=false AND  alum.anio_escolar=(CAST(ciclo.nombre AS SIGNED)+12-"+anioEgreso+"))"
							+	")"
							+   ") or "; 
				}
			...
			if(dni!=null)
			{
				sql+=" and (alum.datos_personales= pers.id and pers.dni="+dni+")";
				sql2+=" and (alum.datos_personales= pers.id and pers.dni="+dni+")";
			}
			
			if(localidad!=null&&!localidad.equals(""))
			{
				sql+=" and (esc.localidad= loc.id and loc.nombre='"+localidad+"')";
				sql2+=" and (esc.localidad= loc.id and loc.nombre='"+localidad+"')";		
			}
			if(provincia!=null&&!provincia.equals(""))
			{
				sql+=" and (esc.provincia= prov.id and prov.descripcion= '"+provincia+"')";
				sql2+=" and (esc.provincia= prov.id and prov.descripcion= '"+provincia+"')";
			}
			if (idRa!=null) {
				sql+=" and (alum.responsable1="+idRa+" or alum.responsable2="+idRa+")";
				sql2+=" and (alum.responsable1="+idRa+" or alum.responsable2="+idRa+")";
			}
			if(idRr!=null)
			{
				PerfilRR rr=rrSrv.obtenerPerfilRR(idRr);
				List<PerfilEA> listaEas =eaSrv.obtenerEAsPorRR(rr);
				
				sql+=" and (";
				sql2+=" and (";
				
				for (PerfilEA perfilEA : listaEas) {
					sql+=" perfil_ea_id="+perfilEA.getIdPerfilEA()+" or ";
					sql2+=" perfil_ea_id="+perfilEA.getIdPerfilEA()+" or ";
				}
				sql=sql.substring(0, sql.length()-3);
				sql2=sql2.substring(0, sql2.length()-3);
				sql+=") ";
			...
			if(padrino != null){
				...
				sql3 ="select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
					+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
					+ " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
					+ " and alum.beca_id = beca.id AND beca.ciclo=ciclo.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ")";
			}
//			System.out.println("############"+sql3);
			if(nombreAlumno != null && padrino != null){
				aniosEgreso=null;
				if(zona != null){
					sql3 ="select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per, escuela esc, localidad loc, zona_cimientos zona, beca beca,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro"
						+ " where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						+ " and alum.datos_personales = per.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ")"
						+ " and ("+consulta+ ")";
					...
					sql3 ="select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per, beca beca,zona_cimientos zona, "
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro"
						+ " where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista and alum.datos_personales = per.id" 
						+ " and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ") and ("+consulta+ ")";
				}
			...
			if(nombreAlumno != null && padrino == null ){
				aniosEgreso=null;
				if(zona != null){
					sql3 ="select alum.*, sel.*, con.* from  perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per, zona_cimientos zona, escuela esc, localidad loc,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro where alum.id = sel.id_perfil_alumno" 
						+ " and sel.convocatoria = con.id_convocatoria and alum.datos_personales = per.id and alum.escuela = esc.id and esc.localidad = loc.id and sel.id = filtro.id_entrevista"
						+ " and loc.zona = zona.id and ("+consulta+ ")";
				}
					...
					sql3 ="select alum.*, sel.*, con.* from  perfil_alumno alum, "+pers+" entrevista_seleccion sel, convocatoria con, persona per,   "
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro where alum.id = sel.id_perfil_alumno" 
						+ " and sel.convocatoria = con.id_convocatoria and alum.datos_personales = per.id and sel.id = filtro.id_entrevista"
						+ " and ("+consulta+ ")";
				}
			...
			if(anios.size()>0){	
				...
				for(AnioEscolar anio:anios)
				{
					sql3 += "  alum.anio_escolar= " + (anio.getId() -1)+" or ";
				}
			...
			//DMS 10/10 agego logica para filtrar por anio de egreso
			if(aniosEgreso!=null)
			{
				
				sql3+=" AND beca.ciclo=ciclo.id AND   alum.beca_id = beca.id and (";
				for (Integer anioEgreso : aniosEgreso) {
			...
			if(dni!=null)
			{
				sql3+=" and (alum.datos_personales= pers.id and pers.dni="+dni+")";
			}
			
			if(localidad!=null&&!localidad.equals(""))
			{
				sql3+=" and (esc.localidad= loc.id and loc.nombre='"+localidad+"')";
			}
			if(provincia!=null&&!provincia.equals(""))
			{
				sql3+=" and (esc.provincia= prov.id and prov.descripcion= '"+provincia+"')";
			}
			if (idRa!=null) {
				sql3+=" and (alum.responsable1="+idRa+" or alum.responsable2="+idRa+")";
			}
			if(idRr!=null)
			{
				PerfilRR rr=rrSrv.obtenerPerfilRR(idRr);
				...
				for (PerfilEA perfilEA : listaEas) {
					sql3+=" perfil_ea_id="+perfilEA.getIdPerfilEA()+" or ";
				}
				sql3=sql3.substring(0, sql3.length()-3);
				sql3+=") ";
			}
	...
	public Integer obtenerCantidadAlumnosExportar(EstadoAlumno estadoAlumno, 
            ZonaCimientos zona, Convocatoria convocatoria, PerfilPadrino padrino, PerfilEA ea, String nombreAlumno, 
            List<AnioEscolar> anios, Boolean anioAdicional, boolean becadosActivos, String eae, Escuela escuela,
            Integer[] aniosEgreso,Integer dni,String localidad, String provincia, Long idRa, Long idRr) {
		...
		if(padrino == null && nombreAlumno == null ){
			...
			String sql ="select alum.id from perfil_alumno alum, ciclo_programa ciclo, "+pers+prov+respAdul+respReg+" entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
						+ " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						+ " and alum.beca_id = beca.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id";
			
			String sql2 = "select alum.id from perfil_alumno alum, ciclo_programa ciclo, "+pers+prov+respAdul+respReg+" entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, zona_cimientos zona,"
						 + " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
						 + " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						 + " and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id";
			...
			System.out.println("###########: "+sql3);
			if(aniosEgreso!=null)sql3 = "select COUNT(*) as row from ("+ sql  +") contador";
			...
			//	System.out.println("1");
				...
				sql3 ="select COUNT(alum.id) as row from perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con,escuela esc, localidad loc, beca beca, zona_cimientos zona,"
					+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno"
					+ " group by a.id) filtro where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
					+ " and alum.beca_id = beca.id AND beca.ciclo=ciclo.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ")";
			}
			...
			//		System.out.println("2");
					sql3 ="select COUNT(alum.id) as row from perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per, escuela esc, localidad loc, zona_cimientos zona, beca beca,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro"
						+ " where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista"
						+ " and alum.datos_personales = per.id and alum.escuela = esc.id and esc.localidad = loc.id and loc.zona = zona.id and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ")"
						+ " and ("+consulta+ ")";
			...
			//		System.out.println("3");
					sql3 ="select COUNT(alum.id) as row from perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per,  "
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro"
						+ " where alum.id = sel.id_perfil_alumno and sel.convocatoria = con.id_convocatoria and sel.id = filtro.id_entrevista and alum.datos_personales = per.id" 
						+ " and (beca.id=alum.beca_id and beca.padrino = "+padrino.getId()+ ") and ("+consulta+ ")";
				}
			...
			if(nombreAlumno != null && padrino == null ){
				...
				//	System.out.println("4");
					sql3 ="select COUNT(alum.id) as row from perfil_alumno alum, ciclo_programa ciclo, "+pers+" entrevista_seleccion sel, convocatoria con, persona per, zona_cimientos zona, escuela esc, localidad loc,"
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro where alum.id = sel.id_perfil_alumno" 
						+ " and sel.convocatoria = con.id_convocatoria and alum.datos_personales = per.id and alum.escuela = esc.id and esc.localidad = loc.id and sel.id = filtro.id_entrevista"
						+ " and loc.zona = zona.id and ("+consulta+ ")";
				}
				...
				//	System.out.println("5");
					...
					sql3 ="select COUNT(alum.id) as row from perfil_alumno alum,  "+pers+" entrevista_seleccion sel, convocatoria con, persona per, "
						+ " (select a.id, max(es.id) id_entrevista from perfil_alumno a, entrevista_seleccion es where a.id = es.id_perfil_alumno group by a.id) filtro where alum.id = sel.id_perfil_alumno" 
						+ " and sel.convocatoria = con.id_convocatoria and alum.datos_personales = per.id and sel.id = filtro.id_entrevista"
						+ " and ("+consulta+ ")";
					}
