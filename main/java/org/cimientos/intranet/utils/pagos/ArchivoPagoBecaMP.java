 ...
 *
 */
public class ArchivoPagoBecaMP extends ArchivoPago{
	
	private String[] columnas = new String[]{"Nombre", "CUIT", "CVU", "Importe"};
	

	
	public ArchivoPagoBecaMP(List<Pago> pagos) {
		super();
		this.setNombre("MP");
...
//		datos.add(columnas);
		for (Map.Entry<PerfilAlumno, Double> entry : pagosAgrupados.entrySet()) {
			...
			columna[0] = this.getNombre(entry.getKey());
			columna[1] = this.getCuit(entry.getKey());
			columna[2] = this.getNroCuenta(entry.getKey());			
